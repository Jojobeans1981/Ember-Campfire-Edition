...
resource "aws_secretsmanager_secret_version" "backend_runtime" {
  ...
  secret_string = jsonencode({
    DATABASE_URL                = format("postgres://%s:%s@%s:%d/%s?sslmode=require", var.db_username, urlencode(random_password.db.result), aws_db_instance.backend.address, aws_db_instance.backend.port, var.db_name)
