<template>
  <div class="speech-practice">
    ...
    <div v-if="phase === 'retry'" class="status retry">
      Try again! Make the sound louder.
    </div>
...
const { startListening, volume: micVolume, sustainProgress: micProgress, requestMicPermission } = useSpeechRecognition();
...
async function playAndListen() {
  ...
  const result = await startListening(currentPhoneme.value, 5000);
