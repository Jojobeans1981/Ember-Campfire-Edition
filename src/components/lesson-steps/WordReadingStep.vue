...
const { startWordListening, isListening: listening, sustainProgress: micProgress, requestMicPermission, cancelListening } = useSpeechRecognition();
...
async function runItem(item) {
  ...
  const result = await new Promise((resolve) => {
    resolveSkip = resolve;
    startWordListening(item.word, 10000).then(resolve);
  });
  resolveSkip = null;
...
<style scoped>
.word-reading-step { display: flex; flex-direction: column; align-items: center; gap: 1rem; padding: 1rem; }
.prompt-text { font-size: 1rem; color: #aaa; }
.word-display { font-size: 3.5rem; color: #FF8C00; font-weight: bold; letter-spacing: 0.1em; }
...
.segments { display: block; font-size: 1.5rem; letter-spacing: 0.2em; margin-top: 0.3rem; }
